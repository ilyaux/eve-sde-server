...
func main() {
	...
	cacheStore, err := cache.NewMemoryCache(cfg.CacheTTL, cfg.CacheMaxSizeMB)
	if err != nil {
	...
	log.Info().
		...
		Int("max_size_mb", cfg.CacheMaxSizeMB).
		Msg("cache initialized (in-memory)")
	...
	cacheMiddleware := apimiddleware.NewCacheMiddleware(cacheStore, cfg.CacheTTL)
	log.Info().Dur("ttl", cfg.CacheTTL).Msg("caching enabled")
	...
	systemHandler := handlers.NewSystemHandler(db, versionInfo, startedAt)
...
<head>
    ...
    <style>
        ...
        a.button { display: inline-block; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold; }
    </style>
</head>
<body>
    <h1>EVE SDE Server</h1>
    <p>REST and GraphQL API for EVE Online Static Data Export.</p>
    <p><strong>Version:</strong> %s <code>%s</code></p>
    ...
    <ul>
        ...
        <li><code>GET /version</code> - Build metadata</li>
    ...
    <h2>Management</h2>
    <p>
        <a href="/admin" class="button" style="background: #2d3748;">
            Admin Dashboard
        </a>
        <a href="/docs" class="button" style="background: #2f855a; margin-left: 10px;">
            API Documentation
        </a>
    </p>
</body>
</html>
`, versionInfo.Version, versionInfo.Commit, cfg.Port, cfg.Port, cfg.Port)
	})
